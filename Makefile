...
help:
	...
	@echo "  check-nested-modules    - Verify tidy/build/test for tools/modboundary, tools/scaffold, tools/provenanceci, tools/mcpserver, tools/agentcli, examples/external-consumer"
...
check-nested-modules:
	@for dir in tools/modboundary tools/scaffold tools/provenanceci tools/mcpserver tools/agentcli examples/external-consumer; do \
		echo "--- $$dir ---"; \
		(cd "$$dir" && go mod tidy -diff && go build ./... && go test ./...) || exit 1; \
	done
	@echo "ok: nested modules (tools/modboundary, tools/scaffold, tools/provenanceci, tools/mcpserver, tools/agentcli, examples/external-consumer) are tidy and build/test cleanly"
