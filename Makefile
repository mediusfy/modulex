...
help:
	...
	@echo "  check-nested-modules    - Verify tidy/build/test for tools/*, examples/external-consumer, services/prreview"
...
check-nested-modules:
	@for dir in tools/modboundary tools/scaffold tools/provenanceci tools/mcpserver tools/agentcli examples/external-consumer services/prreview; do \
		echo "--- $$dir ---"; \
		(cd "$$dir" && go mod tidy -diff && go build ./... && go test ./...) || exit 1; \
	done
	@echo "ok: nested modules (tools/modboundary, tools/scaffold, tools/provenanceci, tools/mcpserver, tools/agentcli, examples/external-consumer, services/prreview) are tidy and build/test cleanly"
