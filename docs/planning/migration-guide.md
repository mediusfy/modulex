...
optional capabilities defined by `modulex.Starter` and `modulex.Stopper`.
...
- Lifecycle capabilities are explicit: `modulex.Starter` for startup work and
  `modulex.Stopper` for shutdown cleanup.
...
## Renaming single-method capability interfaces to Go's `-er` convention

Several single-method capability interfaces have been renamed so their names
follow the standard Go convention of suffixing single-method interfaces with
`-er` (e.g. `io.Reader`, `io.Closer`). No method signatures changed — only the
interface names.

| Before | After |
| --- | --- |
| `modulex.Startable` | `modulex.Starter` |
| `modulex.Stoppable` | `modulex.Stopper` |
| `modulex.ServiceRegistrar` | `modulex.ServiceRegisterer` |
| `modulex.HealthCheckRegistrar` | `modulex.HealthCheckRegisterer` |
| `modulex.ReadinessRegistrar` | `modulex.ReadinessRegisterer` |
...
var _ modulex.Startable = (*Module)(nil)

func NewService(reg modulex.ServiceRegistrar) *Service { /* ... */ }
```
...
var _ modulex.Starter = (*Module)(nil)

func NewService(reg modulex.ServiceRegisterer) *Service { /* ... */ }
```

### What changed and why

- Renamed for consistency with the rest of the capability interfaces
  (`ServiceResolver`, `EventBusProvider`, ...), which already end in `-er`.
- Purely a type-name change: method sets and behavior are unchanged, so a
  simple find-and-replace of the type name is sufficient to migrate.
- The corresponding `mocks/*.go` files generated by `mockery` were renamed to
  match (e.g. `mocks/Startable.go` -> `mocks/Starter.go`).
...
6. Remove no-op `Start` and `Stop` methods from modules that do not need them;
   add `Start` only for modules that implement `modulex.Starter` and `Stop`
   only for modules that implement `modulex.Stopper`.
